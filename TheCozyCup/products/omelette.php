<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        //$redirectUrl = "/products/omelette.php";
        
        // Dynamic base URL
        $baseUrl = "https://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['SCRIPT_NAME']);
        $redirectUrl = $baseUrl . "/omelette.php";
...
<html lang="en">
<head>
    <title>Product: Omelette</title>
</head>
<body>
    <h1>Omelette</h1>
    <img src="../images/food/omelette.jpeg" alt="Omelette">
    <p>A deliciously fluffy omelette, made with farm-fresh eggs and filled with your choice of vibrant veggies, savory meats, and melted cheese, bringing a burst of flavor to every bite. Perfectly seasoned and lightly golden, this omelette is a wholesome and satisfying choice for breakfast or brunch. With a soft, tender texture and a rich, comforting taste, it’s a delightful way to start your day right or fuel up with a quick, tasty meal anytime.</p>
    <p>Price: $8.00</p>
    ...
    $sql = "SELECT username, comment, is_liked 
            ...
            WHERE product_name = 'Omelette' AND comment IS NOT NULL
            ORDER BY id DESC 
            LIMIT 5";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
