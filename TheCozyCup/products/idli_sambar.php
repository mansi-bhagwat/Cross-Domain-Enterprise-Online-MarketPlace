<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      //  $redirectUrl = "/products/idli_sambar.php";
        
         // Dynamic base URL
        $baseUrl = "https://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['SCRIPT_NAME']);
        $redirectUrl = $baseUrl . "/idli_sambar.php";
...
<html lang="en">
<head>
    <title>Product: Idli Sambar</title>
</head>
<body>
    <h1>Idli Sambar</h1>
    <img src="../images/food/idli_sambar.jpeg" alt="Idli Sambar">
    <p>A beloved South Indian breakfast dish that combines fluffy, steamed rice cakes (idlis) with a flavorful, tangy, and mildly spicy lentil soup (sambar). The idlis are soft and light, made from a fermented batter of rice and lentils, giving them a gentle, slightly sour taste. Paired with sambar, which is simmered with vegetables, tamarind, and a blend of aromatic spices, this dish offers a harmonious balance of textures and flavors. Accompanied by coconut chutney, idli sambar is a nutritious, gluten-free option that’s satisfying and energizing, perfect to start the day on a comforting note.</p>
    <p>Price: $9.50</p>
    ...
    $sql = "SELECT username, comment, is_liked 
            ...
            WHERE product_name = 'Idli Sambar' AND comment IS NOT NULL
            ORDER BY id DESC 
            LIMIT 5";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
